...
namespace ZLGCANDemo
{
    ...
    public partial class CANForm : Form
    {
        ...
        private void CANForm_Load(object sender, EventArgs e)
        {
            ...
            comboBox_LINMode.SelectedIndex = 0;
            comboBox_LIN_Baud.SelectedIndex = 2;
            comboBox_LIN_CheckSUM.SelectedIndex = 1;
            ...
            comboBox_netmode.SelectedIndex = 1;
            textBox_localport.Text = "1080";
            textBox_remoteaddr.Text = "192.168.0.200";
            textBox_remoteport.Text = "8000";
        ...
        private void button_start_Click(object sender, EventArgs e)
        {
            ...
            uint type = kDeviceType[comboBox_device.SelectedIndex].device_type;   //增加CANFDNET滤波,CANFDNET滤波必须在startcan之后进行。
            bool netDevice = type == Define.ZCAN_CANETTCP || type == Define.ZCAN_CANETUDP ||
                             type == Define.ZCAN_CANFDNET_400U_TCP || type == Define.ZCAN_CANFDNET_400U_UDP ||
                             type == Define.ZCAN_CANFDNET_200U_TCP || type == Define.ZCAN_CANFDNET_200U_UDP || type == Define.ZCAN_CANFDNET_800U_TCP ||
                             type == Define.ZCAN_CANFDNET_800U_UDP;
            ...
            if (null == recv_data_thread_)
            {
               ...
               // recv_data_thread_.RecvLINData += this.AddData;
            }
            else
            {
                recv_data_thread_.setChannelHandle(channel_handle_);
        ...
        private void button_close_Click(object sender, EventArgs e)
        {
            ...
            button_StartLIN.Enabled = true;
            button_INITLIN.Enabled = true;
            button_ResetLIN.Enabled = true;
        }
        ...
        private void AddData(ZCAN_LIN_MSG[] data, uint len)
        {
            string text = "";
            for (uint i = 0; i < len; ++i)
            {
                ZCAN_LIN_MSG lin_data = data[i];
                byte chnl = lin_data.chnl;
                byte dataType = lin_data.dataType;
                byte id = (byte)(lin_data.zcanLINData.pid.rawVal & 0x3f); //去掉校验位，获取真实ID
                string dir_ = lin_data.zcanLINData.rxData.dir==0 ? "接收" : "发送";
                byte datalen =lin_data.zcanLINData.rxData.datalen;
                if (dataType == 1)
                {
                    text = String.Format("LIN通道:{0:X1} 接收到LIN错误帧，可能当前报文头没有响应", lin_data.chnl);
                    lock (lock_obj)
                    {
                        list_box_data_.Add(text);
                    }
                ...
                text = String.Format("LIN通道:{0:X1}  {1:G}LIN报文: ID:0x{2:X8}  长度:{3:D1} 数据:", lin_data.chnl, dir_ ,id, datalen);
                for (uint j = 0; j < datalen; ++j)
                {
                    text = String.Format("{0:G}{1:X2} ", text, lin_data.zcanLINData.rxData.data[j]);
                }
                lock (lock_obj)
                {
                    list_box_data_.Add(text);
        ...
        private void EnableSet()
        {
            ...
            bool lin_device = type == Define.ZCAN_USBCANFD_200U;
            comboBox_LINMode.Enabled = lin_device;
            comboBox_LIN_Baud.Enabled = lin_device;
            comboBox_LIN_CheckSUM.Enabled = lin_device;
            label_lin_checksum.Enabled = lin_device;
            label_lin_baud.Enabled = lin_device;
            label_lin_mode.Enabled = lin_device;
            ...
            comboBox_standard2.Enabled = !cloudDevice && !netDevice && !accFilter || canfdnetDevice;
            textBox_startid.Enabled = !cloudDevice && !netDevice && !accFilter||canfdnetDevice;
            textBox_endid.Enabled = !cloudDevice && !netDevice && !accFilter || canfdnetDevice;
            ...
            label_standard2.Enabled = !cloudDevice && !netDevice && !accFilter || canfdnetDevice;
            label_startid.Enabled = !cloudDevice && !netDevice && !accFilter || canfdnetDevice;
            label_endid.Enabled = !cloudDevice && !netDevice && !accFilter || canfdnetDevice;
            ...
            button_LINPublish.Enabled = lin_device;
            ...
            bool lin_slave = lin_device && comboBox_LINMode.SelectedIndex == 0; //lin_mode =0 时设为LIN从站
            button_Transmit_LIN.Enabled = lin_device && !lin_slave;
            //button_LINPublish.Enabled = lin_slave;
        ...
        private void comboBox_LINMode_SelectedIndexChanged(object sender, EventArgs e)
        {
            EnableSet();
        }
        ...
        private void button_INITLIN_Click(object sender, EventArgs e)
        {
           
            ZCAN_LIN_INIT_CONFIG config_ = new ZCAN_LIN_INIT_CONFIG();
            config_.linMode = (byte)comboBox_LINMode.SelectedIndex;
            config_.libBaud = kLINbit[comboBox_LIN_Baud.SelectedIndex];
            config_.chkSumMode = (byte)comboBox_LIN_CheckSUM.SelectedIndex;

            IntPtr pConfig = Marshal.AllocHGlobal(Marshal.SizeOf(config_));
            Marshal.StructureToPtr(config_, pConfig, true);
            lin_channel_handle = Method.ZCAN_InitLIN(device_handle_, (uint)channel_index_, pConfig);
            Marshal.FreeHGlobal(pConfig);
            if (NULL == (int)lin_channel_handle)
            {
                MessageBox.Show("初始化LIN失败，请确认当前硬件/固件版本是否支持LIN通讯", "提示",
                        MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            button_INITLIN.Enabled = false;
          
        }

        private void button_StartLIN_Click(object sender, EventArgs e)
        {
            if (Method.ZCAN_StartLIN(lin_channel_handle)==0)
            {
                MessageBox.Show("启动LIN失败，请确认当前硬件/固件版本是否支持LIN通讯", "提示", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            button_StartLIN.Enabled = false;
            button_ResetLIN.Enabled = true;
            comboBox_LINMode.Enabled = false;
            m_bStart = true;
            if (null == recv_data_thread_)
            {
                recv_data_thread_ = new recvdatathread();
                recv_data_thread_.setLINChannelHandle(lin_channel_handle);
                recv_data_thread_.setStart(m_bStart);
                recv_data_thread_.RecvCANData += this.AddData;
                recv_data_thread_.RecvFDData += this.AddData;
                recv_data_thread_.RecvLINData += this.AddData;
        ...
        private void button_ResetLIN_Click(object sender, EventArgs e)
        {
            if (Method.ZCAN_ResetLIN(lin_channel_handle) == 0)
            {
                MessageBox.Show("复位LIN失败，请确认当前硬件/固件版本是否支持LIN通讯", "提示", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            ...
            button_StartLIN.Enabled = true;
            button_INITLIN.Enabled = true;
            button_ResetLIN.Enabled = false;
            comboBox_LINMode.Enabled = true;
        }

        private void button_Transmit_LIN_Click(object sender, EventArgs e)
        {
            
            byte id = (byte)System.Convert.ToByte(textBox_ID.Text, 16);
            string data = textBox_senddata.Text;
            uint result; //发送的帧数


                ZCAN_LIN_MSG lin_data = new ZCAN_LIN_MSG();
                lin_data.chnl =(byte) channel_index_;
                lin_data.dataType = 0;
                lin_data.zcanLINData.pid.rawVal = id;

                IntPtr ptr = Marshal.AllocHGlobal(Marshal.SizeOf(lin_data));
                Marshal.StructureToPtr(lin_data, ptr, true);
                result = Method.ZCAN_TransmitLIN(lin_channel_handle, ptr, 1);
                Marshal.FreeHGlobal(ptr);
                if (result == 0)
                {
                   MessageBox.Show("发送LIN数据失败", "提示", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                   AddErr();
                }
        }

        private void button_LINPublish_Click(object sender, EventArgs e)
        {
            ZCAN_LIN_PUBLISH_CFG lpc = new ZCAN_LIN_PUBLISH_CFG();
            string data = textBox_senddata.Text;
            lpc.data = new byte[8];
            lpc.ID = (byte)System.Convert.ToByte(textBox_ID.Text, 16);

            lpc.datelen =(byte)SplitData(data, ref lpc.data, LIN_MAX_DLEN);
            uint result; //发送的帧数
            lpc.chkSumMode = 0;//默认，按启动时配置

            IntPtr ptr = Marshal.AllocHGlobal(Marshal.SizeOf(lpc));
            Marshal.StructureToPtr(lpc, ptr, true);
            result = Method.ZCAN_SetLINPublish(lin_channel_handle,ptr,1);
            Marshal.FreeHGlobal(ptr);
            if (result == 1)
            {
               string text = "";
               text = String.Format("添加LIN响应成功");
               lock (lock_obj)
               {
                    list_box_data_.Add(text);
                }
